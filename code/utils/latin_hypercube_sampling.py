...
Scilab:
    Copyright (C) 2012 - 2013 - Michael Baudin,  Maria Christopoulou
    Copyright (C) 2010 - 2011 - INRIA - Michael Baudin
    Copyright (C) 2009 - Yann Collette, Jean-Marc Martinez
    website: forge.scilab.org/index.php/p/scidoe/sourcetree/master/macros
    Converted to Python by Abraham Lee.
"""
...
def lhs(n, samples=None, criterion=None, iterations=None, X_start = None):
    ...
    if X_start is not None:
        H = _lhsclassicplusstart(n, samples, X_start)
...
def _lhsclassicplusstart(n, samples, X_start):
    X_start_orig = X_start
    X_start = np.unique(X_start, axis=0)
    ...
    for j in range(n):
        ...
        a = cut[:samples_plus]
        b = cut[1:samples_plus + 1]
        u = np.random.rand(samples_plus)
        
        rdpoints = np.zeros(samples_plus)
        for k in range(samples_plus):
            rdpoints[k] = u[k]*(b[k]-a[k]) + a[k]
        
        corrected_center = rdpoints
        for value in samplevalues:
            idx = np.argmin(np.abs(corrected_center-value))
            mask = np.ones(len(corrected_center))==1
            mask[idx] = False
